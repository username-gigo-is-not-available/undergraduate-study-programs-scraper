...
class StudyProgramParser(Parser):
    ...
    STUDY_PROGRAM_ACCREDITATION_YEAR: int = 2023
    STUDY_PROGRAMS_LI_SELECTOR: str = '#block-views-akreditacija-2023-block-1 > div > div > div > div > div > ul > li > div'
    ...
    def parse_data(self, *args, **kwargs) -> StudyProgram2023:
    ...
    async def run(self, session: ClientSession,
                  ...
                  executor: Executor | None = None) -> list[StudyProgram2023]:

        tasks: list[Task[tuple[int, str, str]]] = []
        study_programs: list[StudyProgram2023] = []
        ...
        for task in asyncio.as_completed(tasks):
            ...
            study_program: StudyProgram2023 = self.parse_data(page_content=page_content, url=url)
