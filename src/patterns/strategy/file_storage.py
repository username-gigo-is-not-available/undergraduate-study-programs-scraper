...
class LocalFileStorage(FileStorageStrategy):
    ...
    async def save_data(cls, data: list[NamedTuple], output_file_name: Path, column_order: list[str]) -> list[NamedTuple]:
        try:
            with open(f"{Path(StorageConfiguration.OUTPUT_DIRECTORY_PATH / output_file_name)}", "w", newline="", encoding='utf-8') as file:
...
class MinioFileStorage(FileStorageStrategy):
    ...
    async def save_data(cls, data: list[NamedTuple], output_file_name: Path, column_order: list[str]) -> list[NamedTuple]:
        try:
            ...
            logging.info(f"Saving data to MinIO bucket {StorageConfiguration.MINIO_BUCKET_NAME} as {output_file_name}")

            minio_client: Minio = MinioClient.connect()
            await minio_client.put_object(
                bucket_name=StorageConfiguration.MINIO_BUCKET_NAME,
            ...
            logging.error(f"Failed to save data to MinIO bucket {StorageConfiguration.MINIO_BUCKET_NAME}: {e}")
            return []
